...
export interface Ok<Payload> {
  ...
  mapOrElse<V>(fb: () => Promise<V>, fn: (v: Payload) => Promise<V>): Promise<Ok<V>>;
...
export interface Err<Code> {
  ...
  mapOrElse<V>(fb: () => Promise<V>, fn: (v: any) => Promise<V>): Promise<Ok<V>>;
...
export class Result<Payload, Code> {
   ...
   * });
   */
  public async mapErr<C>(fn: (err: Error) => Promise<Error>): Promise<Result<Payload, Code | C>> {
    if (await this.isErr()) {
      const payload = await this.payload;
