...
describe("Result.from", () => {
  ...
  test("creates Err for sync error", async () => {
    ...
    expect(await err.sync()).toEqual(await Result.Err("Something went wrong").sync());
  });
  ...
  test("creates Err for async error", async () => {
    const err = Result.from(Promise.resolve().then(() => new Error("Something went wrong")));
    expect(await err.sync()).toEqual(await Result.Err("Something went wrong").sync());
  });
});
...
describe(".map", () => {
  test("computes `fn` value for new Ok()", async () => {
    ...
    expect(await ok.map(increment).then(v => v.sync())).toEqual(await Result.Ok(2).sync());
  });
...
describe(".mapOrElse", () => {
  test("computes `fn` value for Ok()", async () => {
    ...
    const zero = async () => 0;
    const increment = async (i: number) => i + 1;
    expect(await ok.mapOrElse(zero, increment).then(v => v.ok())).toEqual(Option.Some(2));
  });
  ...
  test("computes `fb` value for Err()", async () => {
    ...
    const zero = jest.fn(async () => 0);
    const increment = async (i: number) => i + 1;
    expect(await err.mapOrElse(zero, increment).then(e => e.ok())).toEqual(Option.Some(0));
  });
});
...
describe(".mapErr", () => {
  ...
  test("computes `fb` value for Err()", async () => {
    ...
    expect(await result.err()).toEqual(Option.Some(new Error("Something went wrong. Booh!")));
  });
});
