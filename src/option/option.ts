import { Result, Ok, Err } from "../internal";

export interface Some<Payload> {
  [Symbol.iterator](): OptionIter<Payload>;
...
export interface None {
  [Symbol.iterator](): OptionIter<any>;
...
export class Option<Payload> {
   ...
   * ```
   */
  public mapOrElse<V, T>(fallback: () => V, fn: (payload: Payload) => T): V | T {
    if (typeof this.payload !== "undefined") {
      return fn(this.payload);
    }
   ...
   * ```
   */
  public andThen<T extends Option<V>, V>(fn: (payload: Payload) => T): T | None {
    if (this.isNone()) {
      return this;
    }
...
export class OptionIter<Payload> implements Iterator<Option<Payload>> {
  opt: Option<Payload>;

  constructor(opt: Option<Payload>) {
    this.opt = opt;
  }

  next(): IteratorResult<Option<Payload>> {
    return {
      done: true,
      value: this.opt.take()
    };
